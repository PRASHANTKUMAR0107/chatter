...
const ChatBox = () => {
  ...
  return (
    <div
      className={clsx(
        "flex h-[calc(100vh-5.25rem)] w-full flex-col gap-4 overflow-y-auto bg-zinc-900/50 sm:h-[80vh] sm:basis-3/4 rounded-xl",
        currentChat ? "" : "hidden sm:flex",
      )}
    >
      <div className="flex flex-none items-center justify-between bg-zinc-700 p-1 rounded-xl">
        <button
        ...
        {messages &&
          messages.map((message, index) => (
            <div
              key={index}
              className={clsx(
                "flex max-w-[50%] grow-0 flex-col rounded-full p-3 px-5",
                message?.senderId === user?._id
                  ? "self-end bg-teal-700"
                  : "self-start bg-teal-950",
              )}
              ref={scroll}
            >
      ...
      <div className="flex w-full items-center justify-between gap-2 bg-zinc-700 p-3">
        ...
        />
        <button
          className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-500 hover:bg-blue-400"
          onClick={() =>
